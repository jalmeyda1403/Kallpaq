        ...
        <div class="modal-dialog modal-xl" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body p-4 bg-secondary" style="height: 75vh; overflow-y: auto;">
                    <div ref="pdfContent" class="pdf-document bg-white shadow mx-auto p-5"
                        ...
                        <div class="row mb-4 pb-3 border-bottom position-relative">
                            <div class="position-absolute" style="right: 15px; top: 0;">
                                <span class="text-muted small font-weight-bold">Versión: {{
                                    formatVersion(hallazgoData.hallazgo_ciclo) }}</span>
                            </div>
                            <div class="col-12 text-center">
                                <img :src="logoUrl" alt="Logo" style="max-height: 80px;" class="mb-3">
                                <h5 class="font-weight-bold text-uppercase text-dark mb-1">Sistema Integrado de Gestión
                                </h5>
                        ...
                        <div class="row mb-4 small">
                            <div class="col-sm-4">
                                ...
                                <address>
                                    <strong>Estado: </strong><span class="badge badge-light border">{{
                                        hallazgoData.hallazgo_estado }}</span><br>
                                    <strong>Identificado: </strong>{{
                                        formatDate(hallazgoData.hallazgo_fecha_identificacion) }}<br>
                                    <strong>Fuente: </strong> {{ hallazgoData.hallazgo_origen }}
                                </address>
                            </div>
                            <div class="col-sm-4">
                                <strong class="text-secondary text-uppercase">Procesos</strong>
                                <address>
                                    <div v-for="proceso in hallazgoData.procesos" :key="proceso.id" class="mb-1">
                                        <i class="fas fa-check-circle text-danger mr-1"></i> {{ proceso.proceso_nombre
                                        }}
                                    </div>
                                </address>
                            </div>
                            <div class="col-sm-4">
                                ...
                                <address>
                                    <div class="mb-1"><strong>Especialista:</strong> {{ hallazgoData.especialista?.name
                                        ||
                                        'No asignado' }}</div>
                                    <div class="mb-1"><strong>Auditor:</strong> {{ hallazgoData.auditor?.name ||
                                        'No asignado' }}</div>
                                </address>
                            </div>
                        </div>
                        ...
                        <div class="row mb-5">
                            <div class="col-6">
                                ...
                                <div class="p-3 bg-light rounded text-muted text-justify small"
                        ...
                        <!-- Causa Raíz (Conditional) -->
                        <div v-if="causaData && ['ncm', 'ncme'].includes((hallazgoData.hallazgo_clasificacion || '').toLowerCase())"
                            class="row mb-5">
                            <div class="col-12">
                                ...
                                <div class="bg-light p-4 rounded border-left" style="border-left: 5px solid #dc3545;">
                                        ...
                                        <ul class="list-unstyled mb-0">
                                            <template v-for="i in 5">
                                                <li v-if="causaData['hc_por_que' + i]" :key="i" class="mb-2 d-flex">
                                                    <span class="badge badge-danger mr-2 mt-1">{{ i }}°</span>
                                                    <span class="text-dark font-italic">"{{ causaData['hc_por_que' + i]
                                                        }}"</span>
                                                </li>
                                            </template>
                                        </ul>
                                    ...
                                    <div v-else class="mb-4 pt-3">
                                        ...
                                        <div class="row">
                                            <div class="col-6 mb-3" v-for="m in ishikawaFields" :key="m.key">
                                                <small class="text-muted text-uppercase font-weight-bold">{{ m.label
                                                    }}</small>
                                                <div class="bg-white p-2 rounded border">{{ causaData[m.key] || 'N/A' }}
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div>
                                        <strong class="text-danger d-block mb-2 text-uppercase small">Causa Raíz
                                            Identificada:</strong>
                                        <p class="mb-0 text-dark font-weight-normal">{{ causaData.hc_resultado }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row mb-5">
                            <div class="col-12">
                                <h5 class="text-danger font-weight-bold mb-3">
                                    {{ ['ncm', 'ncme'].includes((hallazgoData.hallazgo_clasificacion ||
                                        '').toLowerCase()) ? '2.' : '1.' }}
                                    Acciones Definidas
                                </h5>
                                <table class="table table-sm table-hover border">
                                    ...
                                    <tbody>
                                        <tr v-if="accionesData.length === 0">
                                            <td colspan="4" class="text-center text-muted py-3">No hay acciones
                                                registradas.</td>
                                        </tr>
                                        <tr v-for="accion in accionesData" :key="accion.id">
                                            ...
                                            <td>
                                                <span class="badge mb-1"
                                                    :class="accion.accion_tipo == 'correctiva' ? 'badge-danger' : 'badge-warning'">{{
                                                        accion.accion_tipo }}</span>
                                                <div class="small">{{ accion.accion_descripcion }}</div>
                                            </td>
                                            <td>
                                                <div class="font-weight-bold">{{ accion.responsable?.name ||
                                                    accion.accion_responsable }}</div>
                                            </td>
                                            <td class="small">
                                                <div><strong>Inicio:</strong> {{ formatDate(accion.accion_fecha_inicio)
                                                    }}</div>
                                                <div><strong>Fin:</strong> {{
                                                    formatDate(accion.accion_fecha_fin_planificada) }}</div>
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
...
const close = () => { if (modalInstance) modalInstance.hide(); };
const formatDate = (ds) => ds ? new Date(ds).toLocaleDateString('es-ES') : '';
const formatVersion = (c) => (c || 1).toString().padStart(3, '0');
...
const downloadPdf = async () => {
    ...
    const opt = { margin: 10, filename: `Plan_${hallazgoData.value.hallazgo_cod || 'Doc'}.pdf`, image: { type: 'jpeg', quality: 0.98 }, html2canvas: { scale: 2, useCORS: true }, jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' } };
    try { await html2pdf().set(opt).from(pdfContent.value).save(); } finally { generating.value = false; }
};

const printDocument = () => {
    if (!pdfContent.value) return;
    const pw = window.open('', '_blank');
    pw.document.write(`<html><head><title>Plan</title><link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@4.6.2/dist/css/bootstrap.min.css"></head><body>${pdfContent.value.innerHTML}</body></html>`);
    pw.document.close();
