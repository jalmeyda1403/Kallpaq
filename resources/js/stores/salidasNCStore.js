    ...
    actions: {
        ...
        async updateSNC(id, data) {
            ...
            try {
                ...
                const response = await axios.post(route('api.salidas-nc.update', { id }), formData, {
                    headers: {
                        'Content-Type': 'multipart/form-data'
                    }
                });
        ...
        async updateTratamiento(sncId, data) {
            ...
            try {
                ...
                // Agregamos todos los campos de tratamiento
                Object.keys(data).forEach(key => {
                    if (data[key] !== null && data[key] !== undefined) {
                        if (Array.isArray(data[key])) {
                            data[key].forEach((item, index) => {
                ...
                const response = await axios.post(route('api.salidas-nc.update', { id: sncId }), formData, {
                    headers: {
                        'Content-Type': 'multipart/form-data'
                    }
                });
